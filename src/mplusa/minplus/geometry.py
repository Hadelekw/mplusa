...
def point_type(point : Collection, vertices : Collection, indexing_start : int = 0) -> Collection:
    result = [[] for _ in range(len(list(vertices)[0]))]
...
class Polytope:
    ...
    def __init__(self,
                 *faces_collection : Collection,
                 adjust_coordinate_dimensions : bool = True) -> None:
        ...
        for rank, faces in enumerate(faces_collection):
            if not rank:
                ...
                self._validate_vertices_dimension(list(faces))  # TODO: validate validation logic
    ...
    def get_line_segments(self, edge_index : int) -> np.ndarray:
        ...
        for i in range(len(x)):
            if y[i] >= x[i]:
                result.append(add_matrices((y[i] - x[i]) + x, y))  # addition here is tropical multiplication
            else:
                result.append(add_matrices((x[i] - y[i]) + y, x))  # addition here is tropical multiplication
        return np.array(result)
    ...
    def get_envelope(self) -> Polytope:
        pass
